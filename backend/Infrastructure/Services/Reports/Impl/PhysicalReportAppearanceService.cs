...
public class PhysicalReportAppearanceService : IPhysicalReportAppearanceService
{
    private readonly IRepository<PhysicalReportAppearance> _praRepo;
    private readonly IUnitOfWork _uow;

    public PhysicalReportAppearanceService(IRepository<PhysicalReportAppearance> praRepo, IUnitOfWork uow)
    {
        _praRepo = praRepo;
        _uow = uow;
    }
    ...
    public bool Update(EditReportAppearanceDto dto, out string failReason)
    {
        ...
        var addList = new List<PhysicalReportAppearance>();
        var updateList = new List<PhysicalReportAppearance>();
        var updateIds = new List<int>();

        foreach (var item in dto.Appearances)
            if (item.DbId == 0)
            {
                var data = new PhysicalReportAppearance
                {
