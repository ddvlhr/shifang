...
public class TestReportService: ITestReportService
{
    ...
    public TestReportService(IRepository<TestReport> trRepo,
        IUnitOfWork uow,
        IRepository<Group> gRepo,
        IRepository<Data> dRepo,
        IRepository<Core.Entities.Specification> spRepo,
        IRepository<MeasureTypeIndicators> mtiRepo,
        IRepository<TestReportAppearance> traRepo,
        IServiceProvider services,
        IHttpContextAccessor accessor,
        IRepository<WaterRecord> wrRepo,
    ...
    public bool Add(AddReportDto dto)
    {
        ...
        var isEdit = false;
        if (_trRepo.All().Any(c => c.GroupId == dto.GroupId))
        {
            isEdit = true;
            report = _trRepo.All()
    ...
    public bool Add(MetricalDataEditDataDto dto)
    {
        ...
        {
            isEdit = true;
            report = _trRepo.All().FirstOrDefault(c => c.GroupId == dto.GroupId);
        }
        ...
        var appearances = new List<TestReportAppearance>();
            ...
            foreach (var rule in singleRules)
            {
                ...
                if (standard.Equals(0))
                    if (currentAppCount > 0)
                    {
                        ...
                        var appearance = new TestReportAppearance
                        {
                            IndicatorId = rule.Id,
        ...
        var result = isEdit ? _uow.Save() >= 0 : _uow.Save() > 0;
        var oldAppearance = _traRepo.All().Where(c => c.ReportId == report.Id).ToList();
        _traRepo.DeleteRange(oldAppearance);
        foreach (var appearance in appearances) appearance.ReportId = report.Id;

        _traRepo.AddRange(appearances);
        result = _uow.Save() >= 0;
        return result;
    }
    ...
    {
        var data = _trRepo.All().AsNoTracking();
        var roleId = _context.getUserRoleId();
        var canSeeOtherData = _context.getCanSeeOtherData();
    ...
    public ReportStatisticDto GetStatisticInfo(int id, out string failReason)
    {
        ...
        var appearances = _traRepo.All().Include(c => c.Indicator).Where(c => c.ReportId == id).Select(c => new
        {
            c.Indicator.Name,
            c.SubScore
        }).ToList();
            ...
            typeName, out DateTime date)
    {
        ...
        date = DateTime.Now;
        var reports = _trRepo.All().AsNoTracking().Include(c => c.Group).ThenInclude(c => c.Turn)
        ...
        foreach (var g in groupList)
        {
            ...
            var appSubScore = _traRepo.All().Where(c => c.ReportId == report.Id).Sum(c => c.SubScore);
