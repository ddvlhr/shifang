...
[AutoInject(typeof(IDisciplineClassService), InjectType.Scope)]
public class DisciplineClassService : SugarRepository<DisciplineClass>, IDisciplineClassService
{
    public async Task<ResultViewModel<BaseEditDto>> EditAsync(BaseEditDto dto)
    {
        var result = new ResultViewModel<BaseEditDto>();
        var item = new DisciplineClass();
        var modify = dto.Id > 0;
        if (modify)
        {
        ...
        var rows = await base.Context.Storageable<DisciplineClass>(item).ExecuteCommandAsync();
    ...
    {
        var options = await base.Context.Queryable<DisciplineClass>()
    ...
    public async Task<PageViewModel<BaseTableDto>> GetTableAsync(BaseQueryInfoDto dto)
    {
        ...
        var filter = Expressionable.Create<DisciplineClass>()
        ...
        var list = await base.Context.Queryable<DisciplineClass>()
        .Where(filter)
