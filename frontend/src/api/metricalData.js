...
export const getMetricalDataOptions = data => {
  return http.request(`/metricalData/options?specificationId=${data.id}&testDate=${data.testDate}&type=${data.type}`, 'get')
}

export const downloadMetricalInfo = data => {
  return http.request('/metricalData/download/info', 'get', {}, data, { responseType: 'blob' })
}
...
export const getMetricalDataBySpecificationIdAndMeasureType = data => {
  return http.request(`/metricalData/specificationId/${data.specificationId}/measureTypeId/${data.measureTypeId}`, 'get')
}
