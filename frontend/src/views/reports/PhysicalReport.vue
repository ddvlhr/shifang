...
export default {
      ...
      rightButtons: [
        {
          ...
          click: async (id, index, row) => {
            const { data: res } = await this.$api.getPhysicalStatistic(id)
            if (res.meta.code !== 0) {
  ...
  methods: {
    query() {
      ...
      this.getPhysicalReports({ page, size })
    ...
    async getPhysicalReports(params) {
      const { data: res } = await this.$api.getPhysicalReports(
    ...
    async addAppearance() {
      ...
      const { data: res } = await this.$api.getPhysicalAppearances(
        selectedData[0].id
      )
    ...
    async download() {
      const selectedData = this.$refs.table.selectedData
      if (selectedData.length === 0) {
        return this.$message.error('请选择需要导出的报表')
      }
      const id = selectedData[0].id
      const res = await this.$api.downloadPhysicalReport(id)
    ...
    async handleSubmit(data) {
      ...
      const { data: res } = await this.$api.updatePhysicalReportAppearance(data)
      if (res.meta.code !== 0) {
        this.formError = true
