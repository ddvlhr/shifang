  ...
  <div class="main-container">
    ...
    <el-card shadow="never">
      ...
      <ele-table
        ...
        :request-fn="getDisciplineClauseList"
...
export default {
  ...
  methods: {
    ...
    query() {
      this.$utils.queryTable(this, this.getDisciplineClauseList)
    },
    async getDisciplineClauseList(params) {
      const { data: res } = await this.$api.getDisciplineClauseList(
    ...
    async edit(data, that) {
      const { data: res } = await that.$api.getDisciplineClause(data.id)
      if (res.meta.code !== 0) {
    ...
    async handleSubmit(data) {
      const { data: res } = await this.$api.editDisciplineClause(data)
      if (res.meta.code !== 0) {
