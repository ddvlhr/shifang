<template>
  <div class="main-container">
    ...
    <el-card shadow="never">
      <el-row :gutter="10">
        <el-col :span="6">
          <el-input v-model="queryInfo.query" placeholder="请输入关键字" clearable @clear="query">
            <el-button
              slot="append"
              icon="el-icon-search"
              @click="query"
      ...
      <ele-table
        ...
        :request-fn="getCraftIndicatorRules"
        ref="table"
      />
    </el-card>
    <ele-form-dialog
...
export default {
  ...
  methods: {
    ...
    async getCraftIndicatorOptions() {
      const { data: res } = await this.$api.getCraftIndicators()
      if (res.meta.code !== 0) {
        return this.$message.error('获取工艺指标失败: ' + res.data.message)
      }
      this.craftIndicatorOptions = res.data
    },
    async getCraftIndicatorRules(params) {
      const { data: res } = await this.$api.getCraftIndicatorRules(
    ...
    async handleSubmit(data) {
      ...
      const { data: res } = await this.$api.editCraftIndicatorRule(data)
      if (res.meta.code !== 0) {
        this.formError = true
